...
public class AttackView_UIElement : UIElement
{
    ...
    {
        base.OnTap( position );
        if (UserInterface.managedBattle.selectedPlayer.board.GetTileAtWorldPosition( InputController.ConvertToWorldPoint( position, Camera.main.transform.position.y - 0.1f ) ) == null && ( ArtilleryTargeting_UIElement.candidate == null ))
        {
